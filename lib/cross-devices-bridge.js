...
// 初始化变量
const CHANNEL_PREFIX = "CHAT_ROOM_";
let currentRoomChannel = null;
let bridgeCurrentUser = null; // 改名避免冲突
 ...
 */
function init() {
    console.log('初始化跨设备通信桥:', deviceId);
    ...
    if (typeof BroadcastChannel !== 'undefined') {
        ...
        globalChannel.onmessage = function(event) {
            ...
            // 处理全局频道消息
            if (message && message.type === 'ROOM_DISCOVERY' && currentRoomChannel && 
                ...
                globalChannel.postMessage({
                    ...
                    roomId: currentRoomChannel.id || currentRoomChannel.name,
                    timestamp: Date.now()
                });
            }
        };
    ...
    window.addEventListener('beforeunload', function() {
        if (currentRoomChannel && bridgeCurrentUser) {
...
function setupLocalStorageFallback() {
    ...
    window.addEventListener('storage', function(event) {
        if (event.key && event.key.startsWith(CHANNEL_PREFIX) && event.newValue) {
            try {
...
function joinRoom(roomId, username) {
    ...
    bridgeCurrentUser = username;
    const channelId = CHANNEL_PREFIX + roomId;
    
    // 创建或重用广播通道
    if (typeof BroadcastChannel !== 'undefined') {
        try {
            currentRoomChannel = new BroadcastChannel(channelId);
            currentRoomChannel.name = roomId; // 保存原始ID方便调试
            
            currentRoomChannel.onmessage = function(event) {
                const message = event.data;
                // 避免处理自己发送的消息
            ...
            // 回退到localStorage
            currentRoomChannel = { id: channelId, name: roomId };
            setupLocalStorageFallback();
        }
    } else {
...
function leaveCurrentRoom() {
    if (currentRoomChannel && bridgeCurrentUser) {
        ...
        // 关闭通道
        if (typeof BroadcastChannel !== 'undefined' && currentRoomChannel.close) {
            try {
                currentRoomChannel.close();
...
function sendMessageToChannel(message) {
    ...
    try {
        if (typeof BroadcastChannel !== 'undefined' && currentRoomChannel instanceof BroadcastChannel) {
            // 使用BroadcastChannel API
            try {
                currentRoomChannel.postMessage(message);
                console.log('通过BroadcastChannel发送消息成功');
            } catch (e) {
                console.error('通过BroadcastChannel发送消息失败:', e);
                // 回退到localStorage
                const channelData = JSON.stringify(message);
                localStorage.setItem(currentRoomChannel.id || CHANNEL_PREFIX + currentRoomChannel.name, channelData);
                setTimeout(() => {
                    localStorage.removeItem(currentRoomChannel.id || CHANNEL_PREFIX + currentRoomChannel.name);
                }, 100);
            }
        } else {
            // 使用localStorage回退方案
            const channelData = JSON.stringify(message);
            localStorage.setItem(currentRoomChannel.id, channelData);
            // 短暂延迟后移除，触发其他窗口的storage事件
            setTimeout(() => {
                localStorage.removeItem(currentRoomChannel.id);
            }, 100);
        }
        return true;
    } catch (e) {
