...
let bridgeCurrentRoom = null;
let bridgeCurrentUser = null;
let messageHandlers = [];
            ...
            if (shouldProcessMessage(data)) {
                ...
                switch(data.type) {
                    ...
                    case MESSAGE_TYPES.HEARTBEAT:
                        ...
                        // 如果刚加入，发送自己的加入通知
                        if (data.joined && bridgeCurrentRoom && bridgeCurrentRoom === data.roomId && bridgeCurrentUser) {
                            setTimeout(() => {
...
function shouldProcessMessage(data) {
    // 如果没有加入房间，不处理任何消息
    if (!bridgeCurrentRoom) return false;
    
    // 只处理针对当前房间的消息
    return data.roomId === bridgeCurrentRoom;
}
...
function notifyPresenceHandlers(presence) {
    presenceHandlers.forEach(function(handler) {
        try {
            handler(presence);
        } catch (error) {
...
function sendMessage(type, message) {
    if (!bridgeCurrentRoom || !bridgeCurrentUser) {
    ...
    const data = {
        ...
        roomId: bridgeCurrentRoom,
        userName: bridgeCurrentUser,
        message: message,
        timestamp: Date.now(),
        type: type
    };
...
function leaveRoom() {
    if (bridgeCurrentRoom && bridgeCurrentUser) {
        ...
        bridgeCurrentRoom = null;
        bridgeCurrentUser = null;
    }
...
function startHeartbeat() {
    ...
    window._heartbeatTimer = setInterval(() => {
        if (bridgeCurrentRoom) {
            // 发送心跳
            const data = {
                sender: bridgeClientId,
                roomId: bridgeCurrentRoom,
                userName: bridgeCurrentUser || 'unknown',
                timestamp: Date.now(),
